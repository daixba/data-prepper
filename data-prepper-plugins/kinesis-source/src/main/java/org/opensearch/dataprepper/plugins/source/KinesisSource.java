...
import org.opensearch.dataprepper.plugins.source.codec.DefaultCodec;
import org.opensearch.dataprepper.plugins.source.configuration.AwsAuthenticationOptions;
import org.opensearch.dataprepper.plugins.source.metrics.MicrometerMetricFactory;
import org.opensearch.dataprepper.plugins.source.processor.KinesisRecordProcessorFactory;
...
//                .httpClient(asyncHttpClient)
    ...
    public void start(Buffer<Record<Event>> buffer) {
        ...
        ConfigsBuilder configsBuilder = new ConfigsBuilder(
                streamName, applicationName, kinesisClient, dynamoClient, cloudWatchClient, workerIdentifier, processorFactory).tableName(tableName);

        RetrievalConfig retrievalConfig = configsBuilder.retrievalConfig().initialPositionInStreamExtended(InitialPositionInStreamExtended.newInitialPosition(sourceConfig.getInitialPosition())); // Default using Fan-Out
        if (sourceConfig.getConsumerStrategy() == KinesisSourceConfig.ConsumerStrategy.POLLING) {
        ...
        scheduler = new Scheduler(
                ...
                configsBuilder.leaseManagementConfig().billingMode(BillingMode.PROVISIONED).initialLeaseTableWriteCapacity(1).initialPositionInStream(InitialPositionInStreamExtended.newInitialPosition(sourceConfig.getInitialPosition())),
